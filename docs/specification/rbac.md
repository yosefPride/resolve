...
- `require_owner_or_group_admin(member, resource_owner_id)` — the
  owner-or-admin rule: a Contributor may act only on resources they created, a
  Group Admin on any resource in the group. Backs comment deletion, link
  deletion, and reference deletion alike (see docs/api.md, "Comment Rules",
  "Link Rules", "Reference Rules"). Tickets do NOT use this rule — editing or
  deleting a ticket (including status changes) is Group Admin only via
