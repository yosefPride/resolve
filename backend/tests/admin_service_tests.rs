...
async fn setup() -> (Database, AdminService, GroupService, UserRepository, AdminRepository) {
    dotenvy::dotenv().ok();
...
#[tokio::test]
async fn test_deletion_check_no_memberships() {
...
#[tokio::test]
async fn test_deletion_check_blocked_group_with_successor() {
...
#[tokio::test]
async fn test_deletion_check_auto_delete_group() {
    let (db, admin, groups, users, _audit) = setup().await;
    let caller_id = create_user(&users, "sysadmin").await;
    make_system_admin(&db, caller_id).await;
...
#[tokio::test]
async fn test_delete_user_missing_successor_conflict() {
    let (db, admin, groups, users, _audit) = setup().await;
    let caller_id = create_user(&users, "sysadmin").await;
    make_system_admin(&db, caller_id).await;
    let target_id = create_user(&users, "sole-admin").await;
    ...
    let result = admin.delete_user(caller_id, target_id, HashMap::new()).await;
    assert_conflict(result);

    // Nothing was mutated: target user still exists and is still the admin.
    assert!(users.find_by_id(target_id).await.expect("find failed").is_some());
    let members = groups
...
#[tokio::test]
async fn test_delete_user_with_successor_succeeds() {
    ...
    assert!(users.find_by_id(target_id).await.expect("find failed").is_none());
    ...
    let entries = audit.list_audit_log_for_group(group_id).await.expect("list failed");
...
#[tokio::test]
async fn test_delete_user_removes_plain_membership() {
    let (db, admin, groups, users, audit) = setup().await;
    let caller_id = create_user(&users, "sysadmin").await;
    make_system_admin(&db, caller_id).await;
    ...
    assert!(users.find_by_id(target_id).await.expect("find failed").is_none());
    let members = groups.list_members(owner_id, group_id).await.expect("list failed");
    assert_eq!(members.len(), 1);
...
#[tokio::test]
async fn test_delete_user_requires_system_admin() {
    let (_db, admin, _groups, users, _audit) = setup().await;
    let caller_id = create_user(&users, "not-admin").await;
    let target_id = create_user(&users, "target").await;
...
#[tokio::test]
async fn test_delete_user_partial_successors_rejected_atomically() {
    let (db, admin, groups, users, _audit) = setup().await;
    let caller_id = create_user(&users, "sysadmin").await;
    make_system_admin(&db, caller_id).await;
    let target_id = create_user(&users, "sole-admin").await;
...
async fn test_list_users_returns_all_users() {
    ...
    let all_users = admin.list_users(caller_id).await.expect("list_users failed");
    assert_eq!(all_users.len(), 3); // caller + the two created above
}

// 14. list_groups requires System Admin.
#[tokio::test]
async fn test_list_groups_requires_system_admin() {
    let (_db, admin, _groups, users, _audit) = setup().await;
    let caller_id = create_user(&users, "not-admin").await;
...
#[tokio::test]
async fn test_list_groups_returns_all_groups() {
    let (db, admin, groups, users, _audit) = setup().await;
    let caller_id = create_user(&users, "sysadmin").await;
    make_system_admin(&db, caller_id).await;
    let owner_id = create_user(&users, "owner").await;
    ...
    let all_groups = admin.list_groups(caller_id).await.expect("list_groups failed");
