...
#[test]
fn test_find_group_by_id_found() {
    support::runtime().block_on(async {
        let repo = setup().await;
        let group = repo
            .create_group(CreateGroupInput {
...
#[test]
fn test_find_group_by_id_not_found() {
    support::runtime().block_on(async {
        let repo = setup().await;
...
#[test]
fn test_insert_member_duplicate_rejected() {
    support::runtime().block_on(async {
        let repo = setup().await;
        let group_id = oid();
        let user_id = oid();
...
#[test]
fn test_delete_members_by_group() {
    support::runtime().block_on(async {
        let repo = setup().await;
        let group_id = oid();
        let other_group_id = oid();
