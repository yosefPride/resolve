...
mod tests {
    ...
    #[test]
    fn derive_title_keeps_short_message_verbatim() {
        assert_eq!(derive_title("  how do I fix this?  "), "how do I fix this?");
    }

    #[test]
    fn derive_title_truncates_long_message_on_char_boundary() {
        let content = "a".repeat(TITLE_MAX_CHARS + 10);
        let title = derive_title(&content);
        assert_eq!(title.chars().count(), TITLE_MAX_CHARS + 1); // +1 for the "…"
        assert!(title.ends_with('…'));
    }

    #[test]
    fn derive_title_does_not_split_multi_byte_chars() {
        let content = "€".repeat(TITLE_MAX_CHARS + 5);
        let title = derive_title(&content);
